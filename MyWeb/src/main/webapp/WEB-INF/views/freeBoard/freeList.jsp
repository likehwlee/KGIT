...
<body>
					...
					<form action="freeList" method="get" name="searchForm"
						...
						<div class="search-wrap">
							...
							<input type="text" class="form-control search-input"
								name="searchName" value="${pageVO.cri.searchName }"> <select
					...
					<form action="freeList" name="pageForm" id="pageForm">
						<div class="text-center">
							...
							<ul class="pagination pagination-sm">
								...
								<c:forEach var="num" begin="${pageVO.startPage }"
									...
									<li class="${pageVO.pageNum == num ? 'active' : '' }"><a
										href="${num}" onclick="page(${num})">${num }</a></li>
								</c:forEach>
								<!--2.다음버튼 활성화 여부  -->
								<c:if test="${pageVO.next }">
							...
							<input type="hidden" name="pageNum" id="pageNum"
								...
								name="amount" id="amount" value="${pageVO.cri.amount }">
	...
	<script>
		//윈도우 로딩시 실행되는 스크립트함수
		//컨트롤러에서 전달된 msg를 받아올, 공백이 아니라면 msg를 출력
		window.onload = function() {

			//뒤로가기 실행후에 앞으로가기 했을 때 변경된 기록이라면 함수 종료
			if (history.state == '') {
				return;
			}
		...
		//6. 검색후에 검색의 정보를 select박스에 키워드처리, input태그에 키워드처리
		//검색처리 (버튼 클릭시 폼전송)
		...
		function page(num){
			...
			event.preventDefault(); //현재 실행되는 이벤트의 
			
			var pageNum = document.querySelector("#pageForm #pageNum").setAttribute("value",num);//페이지폼의 pageNum값을
