...
public class PageVO {
	...
	private boolean prev;//화면에 그려질 이전 버튼
	private boolean next;//화면에 그졀질 다음 버튼
	...
	public PageVO(Criteria cri, int total) {
		...
		//실제 페이지 마지막 페이지 계산
		//만약 게시물이 63개 밖에 없으면? -> 페이지번호는 7까지 표기되어야 함
		//만약에 게시물이 106개 라면 -> 페이지번호 11까지 표기되어야 함
		//공식= (int)Math.ceil(전체게시글 수/한페이지에 보여지는 데이터 수)
		int realEnd = (int)Math.ceil(total/(double)this.amount);
		
		
		//ex: 152개의 게시물
		//1번 페이지 클릭 시 -> endPage공식=10, realEnd=16
		//보여져야 하는 페이지 번호 =10
